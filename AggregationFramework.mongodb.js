//AGGREGATION IS A POWERFUL FRAMWORK FOR COMPLEX OPERATINOS
...
db.cars.insertMany([
  ...
  },
  {
    ...
    "fuel_type": "Electric",
    "transmission": "Automatic",
    "engine": { "type": "Electric Motor", "battery_capacity": "30.2 kWh", "torque": "245 Nm" },
...
//grouping on the basis of maker values
db.cars.aggregate([
    {$group: {_id : "$maker"}}
]);


//the goups of name will have the count of cars as well
db.cars.aggregate([
    {$group: {
        _id : "$maker",
        TotalCars:{$sum:1}
...
//find the number of cars on the basis of fuel typpe
...
//3 ->                                                              COUNT
//will give me the count of the cars with brand as Hyundai
db.cars.aggregate([
    {
        $match:{
            maker:"Hyundai" 
...
//count number of dielsel and petrol cars of Hyundai brand
//basically fuel type ke hisan se grouping karni hai aur count bataan ahia
...
//4 -                                                            PROJECT
//it helps us to project speciifx columns
//find all the hyundayi cars and only show maker  ,model and fuel_type details
...
//on doing model:-1 we will get values in descending order

//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

//6-                                                              SORT BY COUNT
//it will do the grouping on the basis of maker and print the count as well
db.cars.aggregate([
   {
     $sortByCount:"$maker"
   }
...
//each car has multiple owners , if i want to seperately work on each owner then i will use unwind
//each owner will have a seperate document
